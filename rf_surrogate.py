...
pbar = tqdm(total=n_ensemble, desc='Optimization based on random seeds')
...
#     this for loop ends when all candidates in pool are observed 
    for i in np.arange(len(index_learn)):
        ...
        y_train = s_scaler.fit_transform([[i] for i in y_]).reshape(-1,)
...
#             #TODO: select Acquisiton Function for BO
            ...
            ac_value = PI(X_j, RF_model, y_best)
...
np.save(f'test_run_rf_surrogate_{dataset_name}_PI_xi_0.01.npy', master)
